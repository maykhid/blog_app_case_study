...
class SearchCubit extends Cubit<SearchState> {
  SearchCubit(
      {required GetPostsWithAuthorsUseCase<AuthorsRepository, SearchRepository>
          getPostsWithAuthorsUseCase})
      : _getPostsWithAuthorsUseCase = getPostsWithAuthorsUseCase,
        super(const SearchState.unknown());

  final GetPostsWithAuthorsUseCase<AuthorsRepository, SearchRepository>
      _getPostsWithAuthorsUseCase;

  Future<void> search(String searchTerm) async {
    emit(const SearchState.processing());
    final response = await _getPostsWithAuthorsUseCase(searchTerm);
